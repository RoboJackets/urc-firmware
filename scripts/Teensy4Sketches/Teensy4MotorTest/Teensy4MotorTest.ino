...
// functions
void SetSpeedReferenceCommand(int speedRef);
void GetSpeedFeedbackCommand();
void GetPositionFeedbackCommand();
...
//    printCANMessage(rmsg);
  ...
  if (stateMachineTimer >= STATE_UPDATE_RATE_MS) {
    ...
    // send command
    SetSpeedReferenceCommand(motorSpeed);
  ...
  if (sendTimer >= READ_RATE_MS) {
    ...
    GetSpeedFeedbackCommand();
    GetPositionFeedbackCommand();
  }
...
void SetSpeedReferenceCommand(int speedRef) {
  ...
  CAN_message_t myMsg = createMessage(data);
  printCANMessage(myMsg);
  myCan1.write(myMsg);
